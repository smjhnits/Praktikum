...
$\vec{B}_1$ kann als Zerlegung von zwei zirkular polarisierten
Feldern mit den Frequenzen $+\omega$ und $-\omega$ dargestellt werden.
Liegt die Frequenz nah an der Lamorfrequenz $\omega\ua{L}$, kann der
Beitrag des Feldes mit der Frequenz $-\omega$ vernachlässigt werden.
Auf die Probe wird dann ein Gesamtmagenetfeld
\begin{equation}
  \label{eqn:b_ges}
  \vec{B}\ua{ges} = 
  \begin{pmatrix}
    B_1\cos{\left(\omega t\right)} \\
    B_1\sin{\left(\omega t\right)} \\
    B_0
  \end{pmatrix}.
\end{equation}
