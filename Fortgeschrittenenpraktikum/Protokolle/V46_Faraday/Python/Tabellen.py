...
Latexdocument('Probe1.tex').tabular([a._lambda, a.theta_1_1, a.theta_1_2, a.theta_1, a.theta_1_norm.magnitude, a.theta_1_norm.magnitude - a.hr_theta_norm.magnitude], '{$\lambda / \si{\micro\meter$}} & {$\\theta_1(B_+) / \si{\\rad}$} & {$\\theta_1(B_-) / \si{\\rad}$} & {$\\theta_1 / \si{\\rad}$} & {$\Delta\\theta_\symup{1, Norm} / \si{\\rad\per\milli\meter}$}', [2, 2, 2, 3, 3, 3], caption='Messwerte der dotierten GaAs Probe, mit der Dicke $d = \SI{1.36}{\mm}$. $\\theta_1$ beschreibt den Faraday-Rotationswinkel und \Delta\\theta_\symup{1, Norm} den mit dern Dicke $d$ normierten Wert abzüglich des normierten Faraday-Rotationswinkels der hochreinen Probe.', label='probe1')
Latexdocument('Probe2.tex').tabular([a._lambda, a.theta_2_1, a.theta_2_2, a.theta_2, a.theta_2_norm.magnitude, a.theta_2_norm.magnitude - a.hr_theta_norm.magnitude], '{$\lambda / \si{\micro\meter$}} & {$\\theta_2(B_+) / \si{\\rad}$} & {$\\theta_2(B_-) / \si{\\rad}$} & {$\\theta_2 / \si{\\rad}$} & {$\Delta\\theta_\symup{2, Norm} / \si{\\rad\per\milli\meter}$}', [2, 2, 2, 3, 3, 3], caption='Messwerte der dotierten GaAs Probe, mit der Dicke $d = \SI{1.296}{\mm}$. $\\theta_2$ beschreibt den Faraday-Rotationswinkel und \Delta\\theta_\symup{1, Norm} den mit dern Dicke $d$ normierten Wert abzüglich des normierten Faraday-Rotationswinkels der hochreinen Probe.', label='probe2')
Latexdocument('hr_GaAs.tex').tabular([a._lambda, a.hr_theta_1, a.hr_theta_2, a.hr_theta, a.hr_theta_norm.magnitude], '{$\lambda / \si{\micro\meter$}} & {$\\theta(B_+) / \si{\\rad}$} & {$\\theta(B_-) / \si{\\rad}$} & {$\\theta / \si{\\rad}$} ', [2, 2, 2, 3, 3], caption='Messwerte der reinen GaAs Probe, mit der Dicke $d = \SI{5.11}{\mm}$. $\\theta$ beschreibt den Faraday-Rotationswinkel und $theta\symup{Norm}$ der über die Dicke normierten Faraday-Rotationswinkel.', label='hr')
